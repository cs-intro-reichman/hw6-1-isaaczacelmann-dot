...
public class Runigram {
	...
	public static Color[][] flippedHorizontally(Color[][] image) {
		...
		for (int i = 0; i < numRows; i++) {
			...
			if (numCols % 2 == 1) {
				flipped[i][numCols / 2] = image[i][numCols / 2];
			}
		}
		return flipped;
	}
	...
	public static Color[][] flippedVertically(Color[][] image) {
		...
		for (int j = 0; j < numCols; j++) {
			...
			if (numRows % 2 == 1) {
				flipped[numRows / 2][j] = image[numRows / 2][j];
			}
		}
		return flipped;
	}
	...
	public static Color[][] scaled(Color[][] image, int width, int height) {
		double numCols = image[0].length;
		double numRows = image.length;
		Color[][] scaledImage = new Color[height][width];
		for (int i = 0; i < height; i++) {
			for (int j = 0; j < width; j++) {
				int sourceI = (int) (i * (numRows / height));
				int sourceJ = (int) (j * (numCols / width));
	...
	public static void morph(Color[][] source, Color[][] target, int n) {
		...
		Color[][] ScaledTarget = scaled(target, sourceCols, sourceRows);
